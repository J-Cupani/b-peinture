export const testimonials1 = [
  {
    id: 1,
    imageSrc: "/assets/images/testimonial/final-home--1st.png",
    alt: "Testimonial-image",
    subtitle: "Rainbow-Themes",
    name: "Nevine Acotanza",
    designation: "Chief Operating Officer",
    title: "Android App Development",
    date: "via Upwork - Mar 4, 2015 - Aug 30, 2021",
    rating: 5,
    description:
      "Maecenas finibus nec sem ut imperdiet. Ut tincidunt est ac dolor aliquam sodales. Phasellus sed mauris hendrerit, laoreet sem in, lobortis mauris hendrerit ante. Ut tincidunt est ac dolor aliquam sodales phasellus smauris .",
  },
  {
    id: 2,
    imageSrc: "/assets/images/testimonial/final-home--2nd.png",
    alt: "Testimonial-image",
    subtitle: "Bound - Trolola",
    name: "Jone Duone Joe",
    designation: "Operating Officer",
    title: "Web App Development",
    date: "Upwork - Mar 4, 2016 - Aug 30, 2021",
    rating: 5,
    description:
      "Important fact to nec sem ut imperdiet. Ut tincidunt est ac dolor aliquam sodales. Phasellus sed mauris hendrerit, laoreet sem in, lobortis mauris hendrerit ante. Ut tincidunt est ac dolor aliquam sodales phasellus smauris .",
  },
  {
    id: 3,
    imageSrc: "/assets/images/testimonial/final-home--3rd.png",
    alt: "Testimonial-image",
    subtitle: "Glassfisom",
    name: "Nevine Dhawan",
    designation: "CEO Of Officer",
    title: "Android App Design",
    date: "Fiver - Mar 4, 2015 - Aug 30, 2021",
    rating: 5,
    description:
      "No more question for design. Ut tincidunt est ac dolor aliquam sodales. Phasellus sed mauris hendrerit, laoreet sem in, lobortis mauris hendrerit ante. Ut tincidunt est ac dolor aliquam sodales phasellus smauris .",
  },
  {
    id: 4,
    imageSrc: "/assets/images/testimonial/final-home--4th.png",
    alt: "Testimonial-image",
    subtitle: "NCD - Design",
    name: "Mevine Thoda",
    designation: "Marketing Officer",
    title: "CEO - Marketing",
    date: "Thoda Department - Mar 4, 2018 - Aug 30, 2021",
    rating: 5,
    description:
      "Marcent Of Vanice and treatment. Ut tincidunt est ac dolor aliquam sodales. Phasellus sed mauris hendrerit, laoreet sem in, lobortis mauris hendrerit ante. Ut tincidunt est ac dolor aliquam sodales phasellus smauris .",
  },
  {
    id: 5,
    imageSrc: "/assets/images/testimonial/final-home--5th.png",
    alt: "Testimonial-image",
    subtitle: "Default name",
    name: "Davei Luace",
    designation: "Chief Operating Manager",
    title: "Android App Development",
    date: "via Upwork - Mar 4, 2015 - Aug 30, 2021",
    rating: 5,
    description:
      "When managment is so important. Ut tincidunt est ac dolor aliquam sodales. Phasellus sed mauris hendrerit, laoreet sem in, lobortis mauris hendrerit ante. Ut tincidunt est ac dolor aliquam sodales phasellus smauris .",
  },
];

export const testimonials2 = [
  {
    id: 1,
    name: "@alamin",
    position: "Engineer",
    comment:
      "Ypur plaser well for your template from @mr_faruk There is no is best issueusing it too. It'll really speed up the design and development process :)",
    imgSrc: "/assets/images/client/testimonial-2.jpg",
  },
  {
    id: 2,
    name: "@mr_rasel",
    position: "Engineer",
    comment:
      "Much plaser well for your template from @tabriz! The team is best using it too. It'll really speed up the design and development process ==);.",
    imgSrc: "/assets/images/client/testimonial-2.jpg",
  },
  {
    id: 3,
    name: "@Kabir",
    position: "Engineer",
    comment:
      "thank you as well for your template from @Alamin_Orko! The team is using it too. It'll really speed up the design and development process 💨.",
    imgSrc: "/assets/images/client/testimonial-2.jpg",
  },
  {
    id: 4,
    name: "@Tuhin",
    position: "Engineer",
    comment:
      "thank you as well for your template from @Alamin_Orko! The team is using it too. It'll really speed up the design and development process 🏃.",
    imgSrc: "/assets/images/client/testimonial-2.jpg",
  },
];
